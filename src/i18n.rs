...
const ROWS: &[Row] = &[
    ...
    Row { key: "lan_waiting", en: "Allow port {port} on {iface} in the terminal window…", es: "Permite el puerto {port} en {iface} en la ventana de la terminal…", fr: "Autorisez le port {port} sur {iface} dans la fenêtre du terminal…", de: "Port {port} auf {iface} im Terminalfenster erlauben…", zh: "请在终端窗口中允许 {iface} 上的端口 {port}…", hi: "टर्मिनल विंडो में {iface} पर पोर्ट {port} की अनुमति दें…", ja: "端末ウィンドウで {iface} のポート {port} を許可してください…", ko: "터미널 창에서 {iface} 의 포트 {port} 를 허용하세요…" },
    Row { key: "lan_firewall_timeout", en: "Timed out waiting for the port to be allowed", es: "Se agotó el tiempo esperando la autorización del puerto", fr: "Délai dépassé en attendant l'autorisation du port", de: "Zeitüberschreitung beim Freigeben des Ports", zh: "等待端口被允许时超时", hi: "पोर्ट की अनुमति का समय समाप्त", ja: "ポートの許可を待っている間に時間切れになりました", ko: "포트 허용을 기다리다 시간이 초과되었습니다" },
    Row { key: "lan_window_closed", en: "The port window closed. The firewall rule may still be open.", es: "La ventana del puerto se cerró. La regla del cortafuegos puede seguir abierta.", fr: "La fenêtre du port s'est fermée. La règle du pare-feu est peut-être encore ouverte.", de: "Das Portfenster wurde geschlossen. Die Firewall-Regel ist möglicherweise noch offen.", zh: "端口窗口已关闭。防火墙规则可能仍开着。", hi: "पोर्ट विंडो बंद हो गई। फ़ायरवॉल नियम अभी भी खुला हो सकता है।", ja: "ポートのウィンドウが閉じました。ファイアウォール規則が残っている可能性があります。", ko: "포트 창이 닫혔습니다. 방화벽 규칙이 아직 열려 있을 수 있습니다." },
    Row { key: "lan_no_iface", en: "Could not find a private network interface", es: "No se encontró una interfaz de red privada", fr: "Impossible de trouver une interface réseau privée", de: "Keine private Netzwerkschnittstelle gefunden", zh: "找不到专用网络接口", hi: "निजी नेटवर्क इंटरफ़ेस नहीं मिला", ja: "プライベートなネットワークインターフェースが見つかりません", ko: "사설 네트워크 인터페이스를 찾지 못했습니다" },
    Row { key: "lan_bad_name", en: "This computer's name cannot be used in a link", es: "El nombre de este equipo no puede usarse en un enlace", fr: "Le nom de cet ordinateur ne peut pas figurer dans un lien", de: "Der Name dieses Computers kann nicht in einem Link verwendet werden", zh: "这台电脑的名称不能用在链接里", hi: "इस कंप्यूटर का नाम लिंक में नहीं चल सकता", ja: "このパソコンの名前はリンクに使えません", ko: "이 컴퓨터의 이름은 링크에 쓸 수 없습니다" },
    Row { key: "lan_bind_failed", en: "Could not listen on the local network", es: "No se pudo escuchar en la red local", fr: "Impossible d'écouter sur le réseau local", de: "Auf dem lokalen Netzwerk konnte nicht gehört werden", zh: "无法在本地网络上监听", hi: "स्थानीय नेटवर्क पर सुना नहीं जा सका", ja: "ローカルネットワークで待機できませんでした", ko: "로컬 네트워크에서 대기하지 못했습니다" },
];
